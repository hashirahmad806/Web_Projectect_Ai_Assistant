...
const CARD_STYLES = [
  { border: "rgba(99,102,241,0.25)",  bg: "rgba(99,102,241,0.07)",  dot: "#818cf8" },
  { border: "rgba(6,182,212,0.25)",   bg: "rgba(6,182,212,0.07)",   dot: "#67e8f9" },
  { border: "rgba(16,185,129,0.25)",  bg: "rgba(16,185,129,0.07)",  dot: "#6ee7b7" },
  { border: "rgba(245,158,11,0.25)",  bg: "rgba(245,158,11,0.07)",  dot: "#fcd34d" },
  { border: "rgba(239,68,68,0.20)",   bg: "rgba(239,68,68,0.06)",   dot: "#fca5a5" },
];
...
function EmptyState() {
  return (
    <div className="glass rounded-2xl border border-white/5 flex flex-col items-center justify-center gap-4 py-16 text-center">
      ...
      <div>
        <p className="text-base font-bold text-slate-200">No study history yet</p>
        <p className="text-sm text-slate-500 mt-1 max-w-sm leading-6">
          Start a chat, use voice mode, or upload an image — your study sessions will appear here automatically.
        </p>
      </div>
      <div className="flex flex-wrap gap-2 justify-center mt-1">
        {["💬 Start Chatting", "🎤 Voice Mode", "📷 Upload Image"].map((label) => (
          <span key={label} className="chip chip-purple">{label}</span>
        ))}
      </div>
    </div>
  );
...
export default function HistoryScreen() {
  ...
  return (
    <div className="animate-fade-in space-y-4">
      ...
      <div className="glass-strong rounded-2xl p-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl"
            style={{ background: "linear-gradient(135deg,#d97706,#ea580c)", boxShadow: "0 0 16px rgba(245,158,11,0.4)" }}>
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">All your saved sessions for revision</p>
          </div>
        </div>
      ...
      {!loading && history.length > 0 && (
        <div className="grid grid-cols-3 gap-3">
          <div className="glass rounded-2xl border border-white/5 p-3 text-center">
            <p className="text-xl font-extrabold gradient-text">{history.length}</p>
          ...
          <div className="glass rounded-2xl border border-white/5 p-3 text-center">
            <p className="text-xl font-extrabold gradient-text">
              {history.length > 0
                ? new Date(history[0].updatedAt).toLocaleDateString("en-GB", { day: "2-digit", month: "short" })
                : "—"}
            </p>
      ...
      {/* ── Error ─────────────────────────────────────── */}
      {error && (
        <div className="rounded-2xl border border-red-500/20 p-4 text-sm text-red-400"
              ...
              >
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full"
                    style={{ background: `${style.dot}22`, border: `1px solid ${style.dot}55` }}>
                  ...
                  <span className="chip shrink-0"
                    style={{ background: `${style.dot}22`, color: style.dot, borderColor: `${style.dot}44` }}>
                    Session {idx + 1}
                  </span>
                </div>
                ...
                {item.lastQuestion && (
                  <div className="rounded-xl border border-white/5 px-3 py-2.5"
                    ...
                    <p className="text-xs leading-5 text-slate-300 line-clamp-2">{item.lastQuestion}</p>
                  </div>
                )}
              </article>
            );
      ...
      <div className="glass rounded-2xl border border-white/5 p-4 flex items-start gap-3 text-xs text-slate-500">
        ...
        <p className="leading-5">
          <span className="text-slate-300 font-semibold">Exam Tip:</span> Use Study History to quickly
          revisit concepts before a test. Each session saves your questions and AI answers automatically.
        </p>
      </div>
    </div>
  );
